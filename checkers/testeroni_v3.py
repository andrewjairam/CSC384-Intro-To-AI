...
#     '''
def is_safe(state, row, col, piece):
    '''
    Returns true if a piece is safe from capture
    '''
    up_left = state.board[row-1][col-1]
    up_right = state.board[row-1][col+1]
    down_left = state.board[row+1][col-1]
    down_right = state.board[row+1][col+1]
...
def evaluation(state):
    ...
    if len(generate_successors(state)) == 0: # can do this while looping through pieces above: faster
        if state.turn == 'r':
            return float('-inf') # black wins
        if state.turn == 'b':
            return float('inf') # red wins
        
    return (2*red_kings + reds) - (2*black_kings + blacks)
...
def max_value(state, a, b, depth):
    ...
    v = float('-inf')
    best_action = None
    for action in generate_successors(state): # action is type state
        # v = max(v, min_value(action, a, b, depth+1))s
        temp_v = min_value(action, a, b, depth+1)[0]
        if temp_v >= v:
            v = temp_v
            best_action = action   
        if v >= b:
...
def min_value(state, a, b, depth):
    ...
    for action in generate_successors(state): # action is type state
        ...
        temp_v = max_value(action, a, b, depth+1)[0]
        if temp_v <= v:
            v = temp_v
            best_action = action
        if v <= a:
...
def is_terminal(state, depth):
    ...
    '''
    if depth == 4 or evaluation(state) == float('inf') or evaluation(state) == float('-inf'):
        return True
    return False
...
def solve(state):
    solution = ''
    temp_state = state
    while evaluation(temp_state) != float('inf') or evaluation(temp_state) != float('inf'):
...
if __name__ == '__main__':
    ...
    # v, best_action = max_value(state, float('-inf'), float('inf'), 1)
    # print(v, stringify(best_action))
    # print(evaluation(best_action))
    # v, best_action2 = min_value(best_action, float('-inf'), float('inf'), 1)
    # print(v, stringify(best_action2))
    # my_str = ''
    #print(stringify(best_action))
    # # print(board)
