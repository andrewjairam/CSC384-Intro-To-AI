...
def evaluation(state):
    ...
    if len(generate_successors(state)) == 0:
        if state.turn == 'r':
...
def max_value(state, a, b, depth):
    ...
    if cache_max.get(string_board) != None:
        return cache_max.get(string_board), state
...
def min_value(state, a, b, depth):
    ...
    if cache_min.get(string_board) != None:
        return cache_min.get(string_board), state
